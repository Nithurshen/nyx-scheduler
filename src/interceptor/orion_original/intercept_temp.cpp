...
cudaError_t (*kernel_func)(const void *func, dim3 gridDim, dim3 blockDim,
                           void **args, size_t sharedMem,
                           cudaStream_t stream) = NULL;
cudaError_t (*memcpy_func)(void *dst, const void *src, size_t count,
                           enum cudaMemcpyKind kind) = NULL;
cudaError_t (*memcpy_async_func)(void *dst, const void *src, size_t count,
...
cudaError_t (*memset_async_func)(void *devPtr, int value, size_t count,
                                 cudaStream_t stream) = NULL;

cudnnStatus_t (*cudnn_setstream_func)(cudnnHandle_t handle,
                                      cudaStream_t streamId);
cudnnStatus_t (*cudnn_backend_func)(
    cudnnHandle_t handle, const cudnnBackendDescriptor_t executionPlan,
    const cudnnBackendDescriptor_t varianPack) = NULL;
cudnnStatus_t (*cudnn_conv_func)(
    ...
    const cudnnTensorDescriptor_t yDesc, void *y) = NULL;
cudnnStatus_t (*cudnn_bnorm_func)(
    ...
    size_t reserveSpaceSizeInBytes) = NULL;
cudnnStatus_t (*cudnn_bnorm_infer_func)(
    ...
    const void *estimatedVariance, double epsilon) = NULL;
cudnnStatus_t (*cudnn_rnn_func)(
    ...
    size_t workSpaceSizeInBytes) = NULL;
cudnnStatus_t (*cudnn_rnn_train_func)(
    ...
    size_t reserveSpaceSizeInBytes) = NULL;
cublasStatus_t (*cublas_sgemm_func)(
    cublasHandle_t handle, cublasOperation_t transa, cublasOperation_t transb,
    int m, int n, int k, const float *alpha, const float *A, int lda,
    const float *B, int ldb, const float *beta, float *C, int ldc) = NULL;
cublasStatus_t (*cublas_sgemm_strided_func)(
    cublasHandle_t handle, cublasOperation_t transa, cublasOperation_t transb,
    int m, int n, int k, const float *alpha, const float *A, int lda,
    long long int strideA, const float *B, int ldb, long long int strideB,
    const float *beta, float *C, int ldc, long long int strideC,
    int batchCount) = NULL;
cublasStatus_t (*cublas_lt_matmul_func)(
...
cudaError_t cudaMemcpyAsync(void *dst, const void *src, size_t count,
  ...
  DEBUG_PRINT("[IDX: %d] Caught cudaMemcpyAsync! src is %p, dst is %p, size is "
              "%d, stream is %d\n",
  ...
  if (idx < *num_total_clients) {
    ...
    // CHECK_CUDA_ERROR(cudaDeviceSynchronize());
    err = (*memcpy_async_func)(
        dst, src, count, kind,
        stream); // TODO: not sure about which stream to use here
    CHECK_CUDA_ERROR(err);
  }
...
cudaError_t cudaLaunchKernel(const void *func, dim3 gridDim, dim3 blockDim,
  ...
  if (idx < *num_total_clients) {
    ...
    DEBUG_PRINT("[INTERCEPTER-CATCH-%d]-[%d] Caught cudaLaunchKernel, block "
                "size: %d, grid size: %d\n",
                idx, func_indexes[idx], block_size, grid_size);

    new_kernel_record = {func,      gridDim, blockDim, args,
                         sharedMem, stream,  false,    0};
    union func_data new_func_data;
    ...
    // DEBUG_PRINT("[INTERCEPTER] about to call cudaLaunchKernel with func
    // %p\n", func);
