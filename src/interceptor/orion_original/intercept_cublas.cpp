...
cublasStatus_t cublasSgemm_v2(cublasHandle_t handle, cublasOperation_t transa,
  ...
  if (idx < *num_total_clients) {
    ...
    DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught cublasSgemm_v2, handle is %p, "
                "index %d, m is %d, n is %d, k is %d\n",
    ...
    status = (*cublas_sgemm_func)(handle, transa, transb, m, n, k, alpha, A,
...
cublasStatus_t cublasSgemm(cublasHandle_t handle, cublasOperation_t transa,
  ...
  if (idx < *num_total_clients) {
    ...
    pthread_mutex_lock(mutexes[idx]);
    DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught cublasSgemm, handle is %p, "
                "index %d, m is %d, n is %d, k is %d\n",
                func_indexes[idx], handle, idx, m, n, k);
    kqueues[idx]->push(new_record);
    func_indexes[idx] += 1;
...
cublasLtMatmul(cublasLtHandle_t lightHandle, cublasLtMatmulDesc_t computeDesc,
  ...
  cublasLtMatmul_record matmul_record = {
      lightHandle, computeDesc, alpha, A,         Adesc,
      B,           Bdesc,       beta,  C,         Cdesc,
      D,           Ddesc,       algo,  workspace, workspaceSizeInBytes,
      stream};
  ...
  if (idx < *num_total_clients) {
    ...
    pthread_mutex_lock(mutexes[idx]);
    // DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught cublasLtMatmul, handle is
    ...
    *(void **)(&cublas_lt_matmul_func) = dlsym(RTLD_NEXT, "cublasLtMatmul");
    ...
    status = (*cublas_lt_matmul_func)(lightHandle, computeDesc, alpha, A, Adesc,
...
cublasSgemmStridedBatched(cublasHandle_t handle, cublasOperation_t transa,
  ...
  if (idx < *num_total_clients) {
    ...
    pthread_mutex_lock(mutexes[idx]);
    DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught cublasSgemmStridedBatched, "
                "handle is %p\n",
    ...
    if (cublas_sgemm_strided_func == NULL) {
      *(void **)(&cublas_sgemm_strided_func) =
          dlsym(RTLD_NEXT, "cublasSgemmStridedBatched");
      assert(cublas_sgemm_strided_func != NULL);
    }

    status = (*cublas_sgemm_strided_func)(
