...
cudnnStatus_t cudnnBackendExecute(cudnnHandle_t handle,
  ...
  // record
  cudnnBackend_record new_backend_record = {handle, executionPlan, varianPack};
  ...
  if (idx < *num_total_clients) {
    ...
    DEBUG_PRINT("[INTERCEPTER-CATCH-%d]-[%d] Caught cudnnBackendExecute, CUDNN "
                "handle is %p\n",
...
cudnnConvolutionForward(cudnnHandle_t handle, const void *alpha,
  ...
  cudnnConvolutionForward_record new_conv_record = {
      handle, alpha,    xDesc, x,         wDesc,
      w,      convDesc, algo,  workSpace, workSpaceSizeInBytes,
      beta,   yDesc,    y};
  union func_data new_func_data;
  ...
  if (idx < *num_total_clients) {
    ...
    DEBUG_PRINT("[INTERCEPTER-CATCH-%d]-[%d] Caught cudnnConvolutionForward, "
                "CUDNN handle is %p\n",
                idx, func_indexes[idx], handle, idx);
    ...
    if (cudnn_conv_func == NULL) {
      *(void **)(&cudnn_conv_func) =
          dlsym(RTLD_NEXT, "cudnnConvolutionForward");
      assert(cudnn_conv_func != NULL);
    }

    status =
        (*cudnn_conv_func)(handle, alpha, xDesc, x, wDesc, w, convDesc, algo,
...
cudnnStatus_t cudnnBatchNormalizationForwardTrainingEx(
  ...
  if (idx < *num_total_clients) {
    ...
    pthread_mutex_lock(mutexes[idx]);
    DEBUG_PRINT("[INTERCEPTER-CATCH-%d]-[%d] Caught "
                "cudnnBatchNormalizationForwardTrainingEx, handle is %p\n",
    ...
    if (cudnn_bnorm_func == NULL) {
      *(void **)(&cudnn_bnorm_func) =
          dlsym(RTLD_NEXT, "cudnnBatchNormalizationForwardTrainingEx");
      assert(cudnn_bnorm_func != NULL);
    }
    status = (*cudnn_bnorm_func)(
        handle, mode, bnOps, alpha, beta, xDesc, xData, zDesc, zData, yDesc,
        yData, bnScaleBiasMeanVarDesc, bnScaleData, bnBiasData,
        exponentialAverageFactor, resultRunningMeanData,
...

{
  ...
  if (idx < *num_total_clients) {
    ...
    pthread_mutex_lock(mutexes[idx]);
    DEBUG_PRINT(
        "[INTERCEPTER-CATCH]-[%d] Caught "
        "cudnnBatchNormalizationForwardInference, handle is %p, index is %d\n",
    ...
    if (cudnn_bnorm_infer_func == NULL) {
      *(void **)(&cudnn_bnorm_infer_func) =
          dlsym(RTLD_NEXT, "cudnnBatchNormalizationForwardInference");
      assert(cudnn_bnorm_infer_func != NULL);
    }

    status = (*cudnn_bnorm_infer_func)(
        handle, mode, alpha, beta, xDesc, x, xDesc, y, bnScaleBiasMeanVarDesc,
...
cudnnStatus_t cudnnRNNForwardInference(
  ...
  if (idx < *num_total_clients) {
    ...
    cudnnTensorDescriptor_t *xDesc_new =
        (cudnnTensorDescriptor_t *)malloc(sizeof(cudnnTensorDescriptor_t));
    *xDesc_new = *xDesc;

    cudnnTensorDescriptor_t *yDesc_new =
        (cudnnTensorDescriptor_t *)malloc(sizeof(cudnnTensorDescriptor_t));
    *yDesc_new = *yDesc;

    cudnnRNNForwardInf_record rnn_record = {handle,
                                            rnnDesc,
                                            seqLength,
                                            xDesc_new,
                                            x,
    ...
    pthread_mutex_lock(mutexes[idx]);
    DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught cudnnRNNForwardInference, "
                "handle is %p, index is %d\n",
                func_indexes[idx], handle, idx);
    kqueues[idx]->push(new_record);
    func_indexes[idx] += 1;
    pthread_mutex_unlock(mutexes[idx]);
    ...
    if (cudnn_rnn_func == NULL) {
      *(void **)(&cudnn_rnn_func) =
          dlsym(RTLD_NEXT, "cudnnRNNForwardInference");
      assert(cudnn_rnn_func != NULL);
    }

    status = (*cudnn_rnn_func)(handle, rnnDesc, seqLength, xDesc, x, hxDesc, hx,
...
cudnnRNNForwardTraining(cudnnHandle_t handle,
  ...
  if (idx < *num_total_clients) {
    ...
    *yDesc_new = *yDesc;
    ...
    cudnnRNNForwardTraining_record rnn_record = {handle,
                                                 rnnDesc,
                                                 seqLength,
                                                 xDesc,
                                                 x,
    ...
    pthread_mutex_lock(mutexes[idx]);
    DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught cudnnRNNForwardTraining, "
                "handle is %p, index is %d\n",
                func_indexes[idx], handle, idx);
    ...
    if (cudnn_rnn_train_func == NULL) {
      *(void **)(&cudnn_rnn_train_func) =
          dlsym(RTLD_NEXT, "cudnnRNNForwardTraining");
      assert(cudnn_rnn_train_func != NULL);
    }

    status = (*cudnn_rnn_train_func)(
...
cudnnStatus_t cudnnBatchNormalizationBackwardEx(
  ...
  cudnnBatchNormalizationBackwardEx_record record = {handle,
                                                     mode,
                                                     bnOps,
                                                     alphaDataDiff,
                                                     betaDataDiff,
  ...
  if (idx < *num_total_clients) {
    ...
    pthread_mutex_lock(mutexes[idx]);
    DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught "
                "cudnnBatchNormalizationBackwardEx!Index is %d\n",
    ...
    if (cudnn_bnorm_bw_func == NULL) {
      *(void **)(&cudnn_bnorm_bw_func) =
          dlsym(RTLD_NEXT, "cudnnBatchNormalizationBackwardEx");
...
cudnnStatus_t cudnnConvolutionBackwardData(
  ...
  if (idx < *num_total_clients) {
    cudnnConvolutionBackwardData_record record = {
        handle, alpha,    wDesc, w,         dyDesc,
        dy,     convDesc, algo,  workSpace, workSpaceSizeInBytes,
        beta,   dxDesc,   dx};
    ...
    pthread_mutex_lock(mutexes[idx]);
    DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught "
                "cudnnConvolutionBackwardData!Index is %d\n",
                func_indexes[idx], idx);
    ...
    if (cudnn_conv_bw_data_func == NULL) {
      *(void **)(&cudnn_conv_bw_data_func) =
          dlsym(RTLD_NEXT, "cudnnConvolutionBackwardData");
      assert(cudnn_conv_bw_data_func != NULL);
    }
    status = (*cudnn_conv_bw_data_func)(handle, alpha, wDesc, w, dyDesc, dy,
...
cudnnStatus_t cudnnConvolutionBackwardFilter(
  ...
  if (idx < *num_total_clients) {
    ...
    pthread_mutex_lock(mutexes[idx]);
    DEBUG_PRINT("[INTERCEPTER-CATCH]-[%d] Caught "
                "cudnnConvolutionBackwardFilter!Index is %d\n",
                func_indexes[idx], idx);
    kqueues[idx]->push(new_record);
    func_indexes[idx] += 1;
    pthread_mutex_unlock(mutexes[idx]);
    ...
    if (cudnn_conv_bw_filter_func == NULL) {
      *(void **)(&cudnn_conv_bw_filter_func) =
          dlsym(RTLD_NEXT, "cudnnConvolutionBackwardFilter");
